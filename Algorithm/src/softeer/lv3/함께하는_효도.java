 ...
 *
 * 참조링크: https://non-stop.tistory.com/746
 */
public class 함께하는_효도 {
    ...
    static void dfs(int worker, int work, int y, int x, int m, int sum, List<Pos> workerList, int[][] arr,
        int[] answer) {
        ...
        if (work == 3) {
            if (worker + 1 < m) {
                dfs(worker + 1, 0, workerList.get(worker + 1).y, workerList.get(worker + 1).x, m, sum, workerList, arr,
                    answer);
            }
        } else {
